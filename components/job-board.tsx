...
type Job = { id:string; externalJobId:string; companyId:string; company:string; title:string; location:string; city:string|null; province:string|null; category:string; seniority:string; storedSeniority:string; program:string; employmentType:string|null; workplaceType:string|null; specialization:string|null; datePosted:string|null; deadline:string|null; url:string; detailPath:string; summary:string; descriptionText:string; descriptionStatus:JobRow["description_status"]; recruitingUpdate:RecruitingUpdate|null };
...
export default function JobBoard({mode="home",initialCompany=null,initialRows,landing=null}:{mode?:BoardMode;initialCompany?:string|null;initialRows:JobRow[];landing?:JobLandingContent|null}) {
  ...
  return <main>
    ...
    {mode==="detail"&&detailJob&&<article className="job-detail"><div className="job-detail-main"><CompanyLogo company={detailJob.company} size="large"/><p className="eyebrow"><Link href={companyPath(detailJob.company)}>{detailJob.company}</Link></p><h1>{detailJob.title}</h1><div className="modal-meta"><span><MapPin size={15}/>{detailJob.location}</span><span><BriefcaseBusiness size={15}/>{detailJob.program}</span></div>{detailJob.recruitingUpdate&&<p className="modal-process-note"><strong>{detailJob.recruitingUpdate.label}</strong>{detailJob.recruitingUpdate.note}</p>}<p className="job-detail-summary">{detailJob.summary}</p><dl><div><dt>Firm</dt><dd>{detailJob.company}</dd></div><div><dt>Career level</dt><dd>{detailJob.seniority}</dd></div><div><dt>Program</dt><dd>{detailJob.program}</dd></div>{detailJob.employmentType&&<div><dt>Employment</dt><dd>{detailJob.employmentType}</dd></div>}{detailJob.specialization&&<div><dt>Group</dt><dd>{detailJob.specialization}</dd></div>}{detailJob.workplaceType&&<div><dt>Workplace</dt><dd>{detailJob.workplaceType}</dd></div>}<div><dt>Posted</dt><dd>{fmtFull(detailJob.datePosted)}</dd></div><div><dt>Deadline</dt><dd>{fmtFull(detailJob.deadline)}</dd></div></dl>{detailJob.descriptionStatus==="verified"&&detailJob.descriptionText&&<section className="job-description"><p className="eyebrow">From the employer</p><h2>Role description</h2><div>{detailJob.descriptionText}</div><p className="job-description-source">Verified against the employer posting.</p></section>}<a className="apply-button" href={detailJob.url} target="_blank" rel="noreferrer">Apply on employer site <ArrowUpRight size={18}/></a></div><aside><p className="eyebrow">Explore related opportunities</p><Link href={companyPath(detailJob.company)}>See all {detailJob.company} opportunities <ArrowRight size={15}/></Link>{detailJob.city?.toLowerCase().includes("toronto")&&<Link href="/jobs/investment-banking/toronto">See all Investment Banking Jobs in Toronto <ArrowRight size={15}/></Link>}{detailJob.seniority==="Intern / Co-op"&&<Link href="/jobs/investment-banking/internships">See all Investment Banking Internships in Canada <ArrowRight size={15}/></Link>}{detailJob.storedSeniority==="Analyst"&&<Link href="/jobs/investment-banking/analyst">See all Investment Banking Analyst Jobs in Canada <ArrowRight size={15}/></Link>}{detailJob.storedSeniority==="Associate"&&<Link href="/jobs/investment-banking/associate">See all Investment Banking Associate Jobs in Canada <ArrowRight size={15}/></Link>}<Link href="/jobs">Browse all Investment Banking Jobs in Canada <ArrowRight size={15}/></Link></aside></article>}
    ...
    {selected&&<div className="modal-backdrop" onClick={()=>setSelected(null)}><section className="job-modal" onClick={e=>e.stopPropagation()} role="dialog" aria-modal="true"><button className="modal-close" onClick={()=>setSelected(null)} aria-label="Close"><X/></button><CompanyLogo company={selected.company} size="large"/><p className="eyebrow">{selected.company}</p><h2>{selected.title}</h2><div className="modal-meta"><span><MapPin size={15}/>{selected.location}</span><span><BriefcaseBusiness size={15}/>{selected.program}</span></div>{selected.recruitingUpdate&&<p className="modal-process-note"><strong>{selected.recruitingUpdate.label}</strong>{selected.recruitingUpdate.note}</p>}<p className="modal-summary">{selected.summary}</p><dl><div><dt>Field</dt><dd>{selected.category}</dd></div><div><dt>Level</dt><dd>{selected.seniority}</dd></div><div><dt>Deadline</dt><dd>{fmt(selected.deadline)}</dd></div></dl><Link className="detail-link" href={selected.detailPath}>View full job details</Link><a className="apply-button" href={selected.url} target="_blank" rel="noreferrer">Apply on employer site <ArrowUpRight size={18}/></a></section></div>}
  </main>;
}
