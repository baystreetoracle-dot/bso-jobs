...
export default function JobBoard({mode="home",initialCompany=null}:{mode?:"home"|"jobs"|"companies";initialCompany?:string|null}) {
  ...
  return <main>
    ...
    {mode==="home"&&<section className="logo-marquee" aria-label="Firms across the Bay Street community"><p>Followed across the Street</p><div className="logo-marquee-window"><div className="logo-marquee-track">{[0,1].map(copy=><div className="logo-marquee-group" key={copy} aria-hidden={copy===1}>{marqueeFirms.map(company=><div className="logo-marquee-item" key={`${copy}-${company}`}><CompanyLogo company={company}/><span>{company}</span></div>)}</div>)}</div></div></section>}
