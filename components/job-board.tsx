...
const marqueeLogos = [
  {name:"Atlas Partners",src:"/banner-logos/atlas-partners.webp",compact:true},
  ...
  {name:"INFOR Financial",src:"/banner-logos/infor-financial.webp",compact:true},
  {name:"Jefferies",src:"/banner-logos/jefferies.webp",compact:true},
  ...
  {name:"Stifel",src:"/banner-logos/stifel.webp",compact:true},
...
export default function JobBoard({mode="home",initialCompany=null}:{mode?:"home"|"jobs"|"companies";initialCompany?:string|null}) {
  ...
  return <main>
    ...
    {mode==="home"&&<section className="logo-marquee" aria-label="Firms across the Bay Street community"><p>Trusted by leaders across Bay Street</p><div className="logo-marquee-window"><div className="logo-marquee-track">{[0,1].map(copy=><div className="logo-marquee-group" key={copy} aria-hidden={copy===1}>{marqueeLogos.map(logo=><div className="logo-marquee-item" key={`${copy}-${logo.name}`}><Image className={logo.compact?"compact":""} src={logo.src} alt={`${logo.name} logo`} width={240} height={96}/></div>)}</div>)}</div></div></section>}
    ...
    {mode==="home"&&<section className="newsletter-section" id="newsletter"><Reveal><div className="newsletter-inner"><div className="newsletter-icon"><Mail/></div><div><p className="eyebrow light">The BSO briefing</p><h2>The roles and intelligence that matter—once a week.</h2><p>Join 40,000+ readers across Canada&apos;s capital-markets community.</p></div><form onSubmit={e=>{e.preventDefault();setSubscribed(true)}}>{subscribed?<p className="signup-success" aria-live="polite">You&apos;re on the list. Welcome to BSO.</p>:<><input type="email" placeholder="Your email address" aria-label="Email address" required/><button>Join the briefing <ArrowRight size={17}/></button><small>Free. High signal. Unsubscribe anytime.</small></>}</form></div></Reveal></section>}
