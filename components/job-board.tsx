...
export default function JobBoard({mode="home",initialCompany=null,initialRows,landing=null}:{mode?:BoardMode;initialCompany?:string|null;initialRows:JobRow[];landing?:JobLandingContent|null}) {
  ...
  const featuredJobs=useMemo(()=>{const editorial=loadedJobs.filter(({featured})=>featured).map(({job})=>job).sort((a,b)=>{const left=featuredCompanyOrder.indexOf(a.company),right=featuredCompanyOrder.indexOf(b.company);return (left<0?999:left)-(right<0?999:right)});const selectedIds=new Set(editorial.map(job=>job.id));return [...editorial,...jobs.filter(job=>!selectedIds.has(job.id))].slice(0,3)},[jobs,loadedJobs]);
