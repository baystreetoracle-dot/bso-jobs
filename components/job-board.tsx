...
type Job = { company:string; title:string; location:string; category:string; seniority:string; program:string; deadline:string|null; url:string; summary:string; recruitingUpdate:RecruitingUpdate|null };
...
export default function JobBoard({mode="home",initialCompany=null}:{mode?:"home"|"jobs"|"companies";initialCompany?:string|null}) {
  ...
  return <main>
    ...
    {selected&&<div className="modal-backdrop" onClick={()=>setSelected(null)}><section className="job-modal" onClick={e=>e.stopPropagation()} role="dialog" aria-modal="true"><button className="modal-close" onClick={()=>setSelected(null)} aria-label="Close"><X/></button><CompanyLogo company={selected.company} size="large"/><p className="eyebrow">{selected.company}</p><h2>{selected.title}</h2><div className="modal-meta"><span><MapPin size={15}/>{selected.location}</span><span><BriefcaseBusiness size={15}/>{selected.program}</span></div>{selected.recruitingUpdate&&<p className="modal-process-note"><strong>{selected.recruitingUpdate.label}</strong>{selected.recruitingUpdate.note}</p>}<p className="modal-summary">{selected.summary}</p><dl><div><dt>Field</dt><dd>{selected.category}</dd></div><div><dt>Level</dt><dd>{selected.seniority}</dd></div><div><dt>Deadline</dt><dd>{fmt(selected.deadline)}</dd></div></dl><a className="apply-button" href={selected.url} target="_blank" rel="noreferrer">Apply on employer site <ArrowUpRight size={18}/></a></section></div>}
  </main>;
}
